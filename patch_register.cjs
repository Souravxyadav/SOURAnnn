...
const register = fs.readFileSync('student-register.html', 'utf8');

const headerFooterStr = `
<body class="bg-paper min-h-screen flex flex-col font-body text-ink-900">
  ...
  <main class="flex-1 flex flex-col items-center justify-start p-4 py-8">
`;

const footerStr = `
  </main>
  <footer class="w-full bg-white border-t border-ink-100 py-6 text-center text-xs text-ink-500 mt-auto">
    &copy; 2026 ScholarLedger. All rights reserved.
  </footer>
</body>
`;

fs.writeFileSync('student-register.html', register.replace(/<body[^>]*>/, headerFooterStr).replace(/<\/body>/, footerStr));
